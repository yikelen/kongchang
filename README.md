...
| **⑤** | **临时播放** | 独立五键：播放 / 停止 / 暂停 / 全屏 / 收起全屏 + 进度 / 音量 |
| **⑥** | **主播放控制** | `上一条` `下一条`，以及预览→播放 / 停止 / 暂停 / 全屏 / 收起全屏 |
| **⑦** | **进度 / 音量** | 主通道进度条与音量；音量本机记住 |

中间还有 **「编辑选中」** 面板（节目单与临时媒体之间）：改名称、备注、媒体路径、循环；选中临时条目时还可改 **分类**。
...
2. 右侧临时媒体按分类备货：`过场`、`颁奖`、`茶歇`（领导突然加环节时从这里点播）  
3. 开场前把 **全屏投影到** 选成「推荐」屏；开场片先 **预览** 看一眼，再 **播放**  
4. 颁奖间隙：节目单停住，右侧点一首颁奖垫乐；讲话时点 **停止**（渐停），避免盖过人声  

**临时区典型用法：** 主持人说「请下一位上台」时多拖了 20 秒——用临时过场乐顶住，不必改节目单。
...
3. 右侧备：`暖场备用`、`过场`、`应急视频`——领导改稿、等人时从临时区补  
...
| `loop` | 否 | 音频默认循环；视频默认否 |
| `category` | 否 | 临时列表分类（`library` 条目常用） |
| `index` / `id` | 否 | 可省略，软件会补 |

- `cues`：节目单，按数组从上到下  
- `library`：临时媒体，不进节目单顺序  
